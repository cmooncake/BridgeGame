            ...
            if (!TypeToDerived.TryGetValue(baseType, out var derivedTypes))
            {
                if (AllTypes.Count == 0)
                {
                    ...
                    for (int iAssembly = 0; iAssembly < assemblies.Length; iAssembly++)
                    {
                        ...
                        {
                            // 一个可选 Editor DLL 的依赖不兼容时，仍保留同程序集里其余可加载类型，
                            // 避免类型选择菜单在扫描 NPOI 等第三方程序集时整体中断。
                            types = exception.Types;
                            Debug.LogWarning(
                                $"Animancer 跳过程序集 {assembly.GetName().Name} 中无法加载的导出类型：" +
                                $"失败数={exception.LoaderExceptions?.Length ?? 0}。");
                        }

                        for (int iType = 0; iType < types.Length; iType++)
                        {
                            var type = types[iType];
                            if (type != null && IsViableType(type))
                                AllTypes.Add(type);
                        }
                    }
